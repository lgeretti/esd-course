...
\end{block}
\pause
\texttt{---global} just means that these settings are valid for all repositories on your machine. See the \texttt{.gitconfig} file in your home directory.
\end{frame}
...
\begin{block}{Create a new file and check the status (Unix)}
\begin{tabular}{ll}
...
\medskip
Note that you can add as many files as you want to the stage area. However, directories are not tracked: only files with their path.
\end{block}
...
\texttt{\$ git log ---pretty=oneline ---abbrev-commit} 
...
\item Each branch holds a reference called \texttt{HEAD} that points to the latest commit for the branch
\item A new branch has \texttt{HEAD} equal to the commit on which it was created
\item Each new commit on a branch moves the \texttt{HEAD} correspondingly 
\end{itemize}
\end{block}
...
A {\em checkout} is an operation that moves the current repository to a commit:

\texttt{\$ git checkout <hash>}

\medskip
where the \texttt{<hash>} is a hash value taken from the commit log. Even a prefix of the hash works, as long as it is unique in the repository. 
\end{block}

\pause
\begin{block}{Checking out can be relative to the \texttt{HEAD} of a branch}
\begin{tabular}{ll}
\texttt{\$ git branch other} & create a new branch \texttt{other} \\
\texttt{\$ git checkout master} & the \texttt{HEAD} of master \\
\texttt{\$ git checkout other} & the \texttt{HEAD} of \texttt{other} \\
\texttt{\$ git checkout master$\sim$} & the commit before \texttt{HEAD} \\
\texttt{\$ git checkout master$\sim$2} & two commits before \texttt{HEAD}
\end{tabular}
\end{block}
\end{frame}
...
If such commit is a \texttt{HEAD}, we can continue adding commits from there.
\begin{itemize}
...
\end{block}
\pause
\begin{block}{The checkout mechanism is very generic and powerful}
You can move to a specific {\em branch} (discussed later) and a position related to the its commit (called the \texttt{HEAD}). This is easier than
\end{block}
\end{frame}
